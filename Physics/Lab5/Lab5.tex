...
table2 = [
  [c * 10**6, t, r3(period(R_0, L_mean, c) * 10**3), 100 * (t - period(R_0, L_mean, c)*10**3) / (period(R_0, L_mean, c)*10**3)]
  for [c, t] in capacitance_period
]
\end{pycode}

\py{latex_table(table2, ['$C$, мкФ', '$T_{\\textup{э}}$, мс', '$T_{\\textup{т}}$, мс', '$\\delta$'])}
